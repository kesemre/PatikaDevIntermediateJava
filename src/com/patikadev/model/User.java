...
public class User {
    ...
    public static ArrayList<User> getList(){
        String query = "SELECT * FROM public.user ORDER BY id ASC";
    ...
    public static boolean delete(int id){
        ...
        try {
            PreparedStatement pr = DBConnector.getInstance().prepareCall(query);
            pr.setInt(1,id);
            return pr.executeUpdate() != -1;
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    public static String searchQuery(String name,String uname,String type){
        String query = "SELECT * FROM public.user WHERE uname LIKE '%{{uname}}%' AND name ILIKE '%{{name}}%'";
    ...
    public static int getPatikaID(String patikaName){
        String query = "SELECT id FROM public.patika WHERE name = ?";
        int id = 0;
        try {
            PreparedStatement pr = DBConnector.getInstance().prepareStatement(query);
    ...
    public static int getUserIdByPatikaName(String patikaName){
        ...
        String query = "SELECT user_id FROM public.course WHERE patika_id = " + id;
        try {
            Statement st = DBConnector.getInstance().createStatement();
            ResultSet rs = st.executeQuery(query);
            while (rs.next()){
